...
export default function PoolsPage() {
  ...
  const loadPoolData = async () => {
    ...
    try {
      ...
      // Sanitize pool history to ensure reward_paid is a number
    const sanitizedHistory = (history || []).map((pool) => ({
      ...pool,
      reward_paid: Number(pool.reward_paid) || 0, // Convert to number, default to 0
    }));

    setPoolHistory(sanitizedHistory);
      //setPoolHistory(history || []);

      // Find current active pool
      const activePool = sanitizedHistory?.find(p => p.status === 'active');
      if (activePool) {
      ...
      if (sanitizedHistory && sanitizedHistory.length > 0) {
        const completed = sanitizedHistory.filter(p => p.status === 'completed');
        const failed = sanitizedHistory.filter(p => p.status === 'failed' || p.status === 'expired' || p.status === 'expired_needs_referrals');
      ...
      const needsReferrals = sanitizedHistory?.some(p => 
        p.status === 'expired' || 
        p.status === 'failed' || 
